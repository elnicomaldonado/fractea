...
# Script para probar transacciones en Mantle Sepolia
# Asegúrate de que este script tiene permisos de ejecución (chmod +x test-sepolia-tx.sh)
...
if [ -z "$1" ]; then
  echo -e "${YELLOW}Ingresa tu clave privada para la wallet de prueba (con 0x al inicio):${NC}"
  read -s PRIVATE_KEY
  echo # Salto de línea después de input
  if [[ ! $PRIVATE_KEY == 0x* ]]; then
    echo -e "${RED}Error: La clave privada debe comenzar con 0x${NC}"
    exit 1
  fi
else
  PRIVATE_KEY=$1
...
node frontend/src/scripts/test-sepolia-transaction.js
